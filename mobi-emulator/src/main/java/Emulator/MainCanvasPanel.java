...
public class MainCanvasPanel extends JPanel implements Runnable, KeyListener, MouseListener, MouseMotionListener {
   ...
   *
   */
  private static final long serialVersionUID = 4118721949741850727L;
  ...
  public int screenType = MainCanvasPanel.CANVAS;
  ...
  private final boolean captureArea = true;
  public Object Quality = RenderingHints.VALUE_INTERPOLATION_NEAREST_NEIGHBOR;
  ...
  private final JLabel infoColorBox;
  private final JLabel infoColorLabel;

  /* Infoframe variables */
  public JFrame infoFrame;
  private final JLabel infoRectLabel;
  private final JLabel infoXYLabel;
  ...
  public MainCanvasPanel(final MobiEmulator maininstance, final int Width, final int Height) {
    ...
    infoFrame.setLocation(getX() + getWidth(), getY());
  ...
  public void run() {
    ...
    while (true) {
      ...
      if (!isCanvasPaintPaused) {
        if (screenType == MainCanvasPanel.SCREEN) {
          try {
            Thread.sleep(50);
          }
  ...
  public void setCanvasPaused(final boolean value) {
  ...
  public void paintComponent(final Graphics g) {
    if (getCurrentDisplayable() == null) { return; }
    ...
    synchronized (getCurrentDisplayable().paintsync) {
      ...
      if (canvasRotation && (canvasRotationAngleMultiplier != 0)) {
        final Graphics2D g2D = (Graphics2D)g; // .createGraphics();
        final AffineTransform at = new AffineTransform();
        final AffineTransform bt = g2D.getTransform();
        final int rotatedDx = Math.abs((int)((getCanvasWidth() - getCanvasHeight()) / canvasZoom) >> 1);
        at.setToTranslation(((canvasRotationAngleMultiplier % 2) != 0) ? rotatedDx
            : 0,
            ((canvasRotationAngleMultiplier % 2) != 0) ? -rotatedDx
                : 0);
      ...
      if (DemoVersion) {
        final Font f = new Font("Arial", Font.BOLD, 20);
        g2.setFont(f);
        final FontMetrics metrics = g2.getFontMetrics();
        final int Width = metrics.stringWidth("DEMO") + 10;
        final int Height = (20) * (mainCanvasPanelHeight / 100);
      ...
      if (isCanvasPaused()) {
        final FontMetrics metrics = g2.getFontMetrics();
        final int Width = metrics.stringWidth("PAUSED") + 10;
        final int Height = (10) * (mainCanvasPanelHeight / 100);
        g2.setColor(Color.RED);
      ...
      if (isInfoVisible || caputurer.captureArea) {
        ...
        g2.drawRect((mousePressedX < mouseDraggedX) ? mousePressedX
            : mouseDraggedX,
            (mousePressedY < mouseDraggedY) ? mousePressedY
                : mouseDraggedY,
            infoSelectedRect.width, infoSelectedRect.height);
      }
    ...
    if ((mainInstance.profilerframe != null) && mainInstance.profilerframe.isVisible()) {
      mainInstance.profilerframe.updateProfiler();
      mainInstance.profilerframe.updateWatchClasses(mainInstance.jarClasses);
    }
    //      if(this.mainInstance.mvf.autoUpdate.isSelected()&&this.mainInstance.mvf.isVisible())
    //      {
    //          this.mainInstance.mvf.run();
    //      }
    if (mainInstance.methodFrame.isVisible()) {
      mainInstance.methodFrame.updateMethodFrame();
    }
  }

  public void FPSLimiter() {
    // TODO Auto-generated method stub
    final long dt = System.currentTimeMillis() - lastRenderTime;
    final long wait = 1000 / mainInstance.fpsValue;
    try {
  ...
  public void mousePressed(final MouseEvent e) {
    ...
    if (!isPointerinRect(mousePressedX, mousePressedY)) { return; }
  ...
  public void mouseReleased(final MouseEvent e) {
    ...
    if (!isPointerinRect(mouseReleasedX, mouseReleasedY)) { return; }
    if (isPointerEventsSupported) {
  ...
  public void mouseDragged(final MouseEvent e) {
    ...
    if (!isPointerinRect(mouseDraggedX, mouseDraggedY)) { return; }
    if (isPointerEventsSupported) {
    ...
    if (isInfoVisible || captureArea) {
      ...
      if (isInfoVisible) {
        infoRectLabel.setText(" Rect " + "X " + infoSelectedRect.x + ", Y " + infoSelectedRect.y + ", W "
            + infoSelectedRect.width + ", H " + infoSelectedRect.height);
        final int Colorvalue = midp_screen_image.getRGB(mouseDraggedX, mouseDraggedY);
        final String color = Integer.toHexString(Colorvalue);
  ...
  public void mouseMoved(final MouseEvent e) {
    // TODO Auto-generated method stub
    if (mainInstance.mainclass == null) { return; }
    if (!isPointerinRect(e.getX(), e.getY())) { return; }
    final int movedx = e.getX();
    final int movedy = e.getY();
    ...
    if (isInfoVisible) {
      if ((movedx >= getCanvasWidth()) || (movedy >= getCanvasHeight())) { return; }
      final int Colorvalue = midp_screen_image.getRGB((int)(movedx / getZoomVal()), (int)(movedy / getZoomVal()));
      final String color = Integer.toHexString(Colorvalue);
      mainInstance.canvasPanel.infoColorLabel.setText(" Color: " + color);
      mainInstance.canvasPanel.infoColorBox.setBackground(new Color(Colorvalue));
    }
  ...
  public void setZoomVal(final float Zoomvalue) {
  ...
  public javax.microedition.lcdui.Displayable getCurrentDisplayable() {
    // TODO Auto-generated method stub
    if (getDisplayType() == MainCanvasPanel.SCREEN) {
      return midp_screen;
    }
    else {
      return midp_canvas;
  ...
  public void setCurrent(final javax.microedition.lcdui.Displayable nextDisplayable) {
    // TODO Auto-generated method stub
    if (getDisplayType() == MainCanvasPanel.SCREEN) {
  ...
  public void setCanvasWidth(final int canvasPanelWidth) {
    // TODO Auto-generated method stub
    mainCanvasPanelWidth = canvasPanelWidth;
  }

  public void setCanvasHeight(final int canvasPanelHeight) {
    // TODO Auto-generated method stub
    mainCanvasPanelHeight = canvasPanelHeight;
  }

  public void updateInfoFramePos(final int mainWindowX, final int mainWindowY) {
    // TODO Auto-generated method stub
    infoFrame.setLocation(new Point((mainWindowX <= 0) ? mainWindowX + (getCanvasWidth() + 2)
        : (mainWindowX - infoFrame.getWidth()), mainWindowY));
  }

  public void updateCanvasSize(final int newWidth, final int newHeight) {
  ...
  void takeScreenShot(final int x, final int y, final int w, final int h, final boolean clipboard) {
    if (caputurer != null) {
  ...
  private int getRotatedX(final int x, final int y) {
    int pointerX = x;
    if (canvasRotation && (canvasRotationAngleMultiplier != 0)) {
  ...
  private int getRotatedY(final int x, final int y) {
    int pointerY = y;
    if (canvasRotation && (canvasRotationAngleMultiplier != 0)) {
      switch (canvasRotationAngleMultiplier) {
        case 0:
  ...
  private boolean isPointerinRect(final int x, final int y) {
