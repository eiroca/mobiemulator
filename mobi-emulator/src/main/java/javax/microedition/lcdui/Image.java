...
public class Image {
  ...
  /**
   * Constructor for creating an Image from an InputStream of image data. Currently only supports
   * PNG data, using the sixlegs.com library.
   */
  Image(final InputStream is) {
    try {
      //
      ...
      try {
        _image = Image.createPng(is); // )png.read(is,true);
      }
   ...
   * @param h
   */
  Image(final int w, final int h) {
    _image = new java.awt.image.BufferedImage(w, h, java.awt.image.BufferedImage.TYPE_INT_ARGB);
    mutable = true;
  ...
  public static Image createImage(final Image image, final int x, final int y, final int width, final int height, final int transform) {
    ...
    final Image midpimage = Image.createImage(WidthHeightReversed ? width
        : height,
        WidthHeightReversed ? height
            : width);
  ...
  public static Image createImage(final byte[] data, final int start, final int len) {
  ...
  public static Image createImage(final InputStream is) {
    return new Image(is);
  }
  ...
  public static Image createImage(final Image source) {
    ...
    final Image copy = new Image(source.getWidth(), source.getHeight());
  ...
  public javax.microedition.lcdui.Graphics getGraphics() {
    ...
    return new Graphics(_image.createGraphics(), _image);
  }
  ...
  private static java.awt.image.BufferedImage createPNG(final byte[] imageData) {
    int width = 0;
    int height = 0;
    int colorType = 0;
    boolean trans = false;
    ...
    try {
      ...
      in.skip(8L);
      while (!done) {
        final int chunk_size = in.readInt();
        final int chunk_name = in.readInt();
    ...
    final BufferedImage image = new BufferedImage(width, height, (trans) ? 2
        : 1);
  ...
  private static boolean isChunk(final int chunkname) {
    for (final int chunk : Image.chunks) {
  ...
  public static Image createRGBImage(final int[] rgb, final int width, final int height, final boolean processAlpha) {
    ...
    rgbDataBuffer = null;
    final Image rgbImage = new Image(image);
    image = null;

    return rgbImage;
  }

  public void getRGB(final int[] rgbData, int offset, final int scanlength, final int x, final int y, final int width, final int height) {
