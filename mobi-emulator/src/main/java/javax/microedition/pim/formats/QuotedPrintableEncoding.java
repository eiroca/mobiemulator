...
public class QuotedPrintableEncoding {
  ...
  public static byte[] fromQuotedPrintable(final String sdata) {
    final ByteArrayOutputStream out = new ByteArrayOutputStream();
    final ByteArrayOutputStream whitespaceAccumulator = new ByteArrayOutputStream();
    final char data[] = sdata.toCharArray();
    ...
    for (int i = 0; i < data.length; i++) {
      if (followingEqualsSign) {
        switch (data[i]) {
          default:
            if (i < (data.length - 1)) {
              final String charCode = sdata.substring(i, i + 2);
              i++;
              try {
