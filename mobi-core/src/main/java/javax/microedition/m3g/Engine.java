...
class Engine {
  ...
  static Object instantiateJavaPeer(final int handle) {
    boolean success;
    ...
    if (peer instanceof Object3D) {
      ...
      for (int index = 0; (len = o3d.getUserParameterValue(index, null)) != -1; index++) {
        ...
        final Integer id = new Integer(o3d.getUserParameterID(index));
  ...
  private static void cleanUpPeerTable() {
    final Enumeration keys = Engine.peerTable.keys();

    while (keys.hasMoreElements()) {
      final Key key = (Key)keys.nextElement();
      final WeakReference ref = (WeakReference)Engine.peerTable.get(key);
  ...
  static void addJavaPeer(final int handle, final Object peer) {
    synchronized (Engine.peerTable) {
      if (Engine.cleanPeerTable) {
        Engine.cleanUpPeerTable();
        Engine.cleanPeerTable = false;
      }
      Engine.peerTable.put(new Key(handle), new WeakReference(peer));
    }
  }

  static Object getJavaPeer(final int handle) {
    Object entry;
    synchronized (Engine.peerTable) {
      if (Engine.cleanPeerTable) {
        Engine.cleanUpPeerTable();
      ...
      Engine.lookup.setKey(handle);

      entry = Engine.peerTable.get(Engine.lookup);
  ...
  static int[] getJavaPeerArrayHandles(final Object[] peers) {
  ...
  static void addXOT(final Object3D[] obj) {
    if (obj != null) {
      for (final Object3D element : obj) {
        Engine.addXOT(element);
      }
    }
  }
}
